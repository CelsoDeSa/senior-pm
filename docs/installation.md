...
Direct Git references in OpenCode's `plugin` array are not a documented installation mechanism. No canonical candidate remote is authorized, so a source clone URL is intentionally unavailable in this preview. From an independently obtained reviewed checkout, build locally and use the package CLI:
...
Inserting final clone URLs during a separately authorized cutover requires a fresh exact-tree content and security review. This document does not claim that any existing repository is the public candidate source.
